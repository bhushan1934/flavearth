    ...
    <div class="container-custom px-4 md:px-6">
        <div class="flex items-center justify-between py-3 md:py-4">
            ...
            <a href="/" class="flex items-center">
                <img src="{{ asset('images/categories/logo.png') }}" alt="Flavearth" class="h-10 md:h-12 w-auto">
            </a>
            ...
            <div class="flex items-center space-x-2 md:space-x-4">
                <a href="#" class="text-gray-700 hover:text-brand transition-colors p-1" aria-label="Search">
                ...
                <a href="{{ route('account') }}" class="text-gray-700 hover:text-brand transition-colors p-1" aria-label="Account">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 md:h-5 md:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                    </svg>
                </a>
                <a href="{{ route('wishlist') }}" class="text-gray-700 hover:text-brand transition-colors p-1 hidden sm:block" aria-label="Wishlist">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 md:h-5 md:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                    </svg>
                </a>
                <a href="{{ route('cart') }}" class="text-gray-700 hover:text-brand transition-colors relative p-1" aria-label="Cart">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 md:h-5 md:w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                    </svg>
                    @if(session()->has('cart_count') && session('cart_count') > 0)
                        <span class="absolute -top-1 -right-1 bg-brand text-white text-xs font-bold rounded-full h-4 w-4 md:h-5 md:w-5 flex items-center justify-center">
                            {{ session('cart_count') }}
                        </span>
                    @endif
                </a>
